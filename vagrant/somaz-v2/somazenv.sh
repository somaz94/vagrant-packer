...
function _base_show_usage() {
  ...
  echo -e "  start     Create training VM Server and Start up"
  echo -e "  stop      Halt all training VM server and [force] option will force stop"
  echo -e "  rebuild   Rebuild all training VM server"
  echo -e "  revert    [somaz] or [somazpc] Revert training VM server"
  echo -e "  snapshot  Create snapshot of current state"
  echo -e ""
  exit 0
}
...
function _vm_stop_status() {
  ...
  virsh_status=$(virsh list --name | grep somazpc || true)

  if [ -n "$virsh_status" ]; then
    echo "Some VM is running. It will stop all VMs"
    for vm_name in $(virsh list --name | grep somazpc); do virsh destroy "$vm_name"; done
    vagrant halt -f
  else
    echo "All VMs are already stopped."
  fi
}
...
function _build_vm() {
  ...
  is_service_network=$(virsh net-list | grep service | awk '{print $1}' || true)

  if [ "$is_service_network" != "service" ]; then
    echo "Service network will be created"
...
function _revert_vm() {
  ...
  echo "All VMs will revert to $1 installed status. This may take about 30min."

  for vm_name in $(virsh list --name --all | grep "$f_name"); do
...
command=$1
arg=$2
win10_state=$(virsh domstate win10 2>/dev/null || echo "unknown")
f_name=${PWD##*/}
...
case "$command" in
  ...
  "start")   _build_vm ;;
  "rebuild")  _rebuild_vm ;;
  "revert")  _revert_vm "$arg" ;;
  "stop")    _stop_vm "$arg" ;;
  *)         _base_show_usage; exit 1 ;;
esac
